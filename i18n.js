const TRANSLATIONS = {
  pl: {
    ...
    miners_price_hint: "Ceny orientacyjne brutto z asicminervalue.com (rynek nowy/używany, przeliczone po bieżącym kursie). Ceny oznaczone „~” to szacunek (interpolacja z podobnych modeli) — dla tego modelu nie było aktywnej oferty. Nie traktuj tego jako oferty zakupu.",
    use_this: "Użyj",
    best_value: (units, name, totalHs) => `💡 Maksymalna moc obliczeniowa: ${units} × ${name} (${totalHs} TH/s łącznie)`,
    min_equipment: (units, name) => `📦 Minimalna ilość sprzętu: ${units} × ${name}`,
  ...
  en: {
    ...
    miners_price_hint: "Rough gross prices from asicminervalue.com (new/used market, converted at the current rate). Prices marked \"~\" are estimated (interpolated from similar models) — no active listing existed for that model. Not a purchase offer.",
    use_this: "Use it",
    best_value: (units, name, totalHs) => `💡 Maximum compute power: ${units} × ${name} (${totalHs} TH/s total)`,
    min_equipment: (units, name) => `📦 Fewest devices: ${units} × ${name}`,
